...
fn flexible_date_parse(text: &str) -> Option<NaiveDate> {
    if let Ok(d) = NaiveDate::parse_from_str(text, "%Y-%m-%d") {
        Some(d)
    } else if let Ok(d) = NaiveDate::parse_from_str(text, "%Y/%b/%d") {
        Some(d)
    } else if let Ok(d) = NaiveDate::parse_from_str(text, "%d.%b.%Y") {
        Some(d)
    } else if let Ok(d) = NaiveDate::parse_from_str(text, "%b.%d.%Y") {
        Some(d)
    } else {
        None
    }
